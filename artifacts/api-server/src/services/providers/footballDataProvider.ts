...
export const FD_COMPETITIONS: Record<string, { code: string; name: string; country: string; emblem: string }> = {
...
export interface LiveScorer {
  ...
  playedMatches: number; leagueSlug: string; leagueName: string;
}

export interface LiveTeam {
  id: number; name: string; shortName: string; crest: string;
  founded: number | null; venue: string | null; coach: string | null;
  website: string | null; leagueSlug: string; leagueName: string; squadSize: number;
}

function slugFromCode(code: string): string {
  return Object.entries(FD_COMPETITIONS).find(([, v]) => v.code === code)?.[0] ?? code.toLowerCase();
}
...
export async function getStandings(slug: string): Promise<LiveStanding[]> {
  ...
  return cached(`fd:standings:${slug}`, TTL.STANDINGS, async () => {
    ...
    if (!isMultiGroup) {
      return (sourceGroups[0]?.table ?? []).map(r => ({
  ...
  return cached(cacheKey, ttl, async () => {
    ...
    const data = await fdFetch(`/competitions/${comp.code}/matches${qs}`) as {
      matches: Array<{
        ...
        minute?: number | null; venue?: string | null;
      }>;
    };
...
export async function getAllLiveMatches(): Promise<LiveMatch[]> {
  return cached("fd:live:all", TTL.LIVE, async () => {
    const data = await fdFetch("/matches?status=LIVE") as {
      matches: Array<{
        id: number;
    ...
    return (data.matches ?? [])
      ...
      .map(m => {
        ...
        return {
          ...
          homeTeam: { id: m.homeTeam.id, name: m.homeTeam.name, shortName: m.homeTeam.shortName ?? m.homeTeam.tla, crest: m.homeTeam.crest },
          awayTeam: { id: m.awayTeam.id, name: m.awayTeam.name, shortName: m.awayTeam.shortName ?? m.awayTeam.tla, crest: m.awayTeam.crest },
          homeScore: null, awayScore: null, status: "upcoming" as const, minute: null,
          matchDate: m.utcDate, matchday: m.matchday,
          venue: m.venue ?? null,
          leagueCode: m.competition.code, leagueName: m.competition.name,
  ...
  return cached(`fd:scorers:${slug}`, TTL.SCORERS, async () => {
    const data = await fdFetch(`/competitions/${comp.code}/scorers?limit=20`) as {
      scorers: Array<{
        player: { id: number; name: string; nationality: string; dateOfBirth: string; position: string | null };
        team: { id: number; name: string; shortName: string; crest: string };
        goals: number; assists: number | null; penalties: number | null; playedMatches: number;
      }>;
    };
    return (data.scorers ?? []).map(s => ({
      player: s.player, team: s.team,
      goals: s.goals, assists: s.assists, penalties: s.penalties,
      playedMatches: s.playedMatches, leagueSlug: slug, leagueName: comp.name,
    }));
  });
}
...
export async function getTeams(slug: string): Promise<LiveTeam[]> {
  const comp = FD_COMPETITIONS[slug];
  if (!comp) throw new Error(`No football-data.org config for ${slug}`);
  return cached(`fd:teams:${slug}`, TTL.TEAMS, async () => {
    const data = await fdFetch(`/competitions/${comp.code}/teams`) as {
      teams: Array<{
        id: number; name: string; shortName: string; crest: string;
        founded: number | null; venue: string | null; website: string | null;
        coach?: { name: string } | null; squad?: Array<unknown>;
      }>;
    };
    return (data.teams ?? []).map(t => ({
      id: t.id, name: t.name, shortName: t.shortName ?? t.name, crest: t.crest,
      founded: t.founded, venue: t.venue, coach: t.coach?.name ?? null,
      website: t.website, leagueSlug: slug, leagueName: comp.name,
