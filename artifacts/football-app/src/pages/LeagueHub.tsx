...
function KnockoutView({ slug, season }: { slug: string; season: number }) {
  ...
  const mainRounds = React.useMemo(
    () => (data?.rounds ?? []).filter(r => r.stage !== "THIRD_PLACE"),
    [data],
  );

  // ── Bracket progression validation (dev-only) ─────────────────────────────
  React.useEffect(() => {
    for (let i = 1; i < mainRounds.length; i++) {
      ...
      for (let q = 0; q < curr.ties.length; q++) {
        ...
        if (tie.sourceAId && expectedSrcA && tie.sourceAId !== expectedSrcA.id) {
          console.error(
            `[Knockout] Invalid bracket progression — ` +
        ...
        if (tie.sourceBId && expectedSrcB && tie.sourceBId !== expectedSrcB.id) {
          console.error(
            `[Knockout] Invalid bracket progression — ` +
